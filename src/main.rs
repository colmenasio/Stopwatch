...
fn main() {
    ...
    // l button
    // r button
    // add/substract time to the target time when inputing with the +- buttons
    let weak = main_window.as_weak();
    ...
    main_window.global::<CLogic>().on_update_time(move || {
        let app = weak.unwrap();
        let logic_module = app.global::<CLogic>();
        let borrowed_countdown = simple_countdown.borrow_mut();

        // TODO add logic to determine which thing to update
        logic_module.set_inactive_str(borrowed_countdown.get_display().into());
    });
